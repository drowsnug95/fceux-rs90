...
#ifdef USE_MXU
/*convert 208px to 160px using MXU instructions by drowsnug */
void downscale_208to160_mxu(uint32_t* __restrict__ src, uint32_t* __restrict__ dst)
{
    ...
    const uint16_t iy=4;
    const uint32_t mask[1]  = {0xF7DEF7DE};   
    ...
    for(int H=0;H<52;H+=1)
    {
        ...
        for(int W=0;W<120;W++) 
        {
            uint32_t* __restrict__ buffer_mem = &src[y<<7] + x;
            S32LDI(xr1,buffer_mem,512); //a=buffer_mem[x];
            S32LDI(xr3,buffer_mem,512); //b=buffer_mem[x+128];
            S32LDI(xr5,buffer_mem,512); //c=buffer_mem[x+128*2];
            S32LDI(xr7,buffer_mem,512); //d=buffer_mem[x+128*3];
            
            //(xr* & 0xF7DEF7DE)>>1
            S32LDD(xr15, mask, 0x0);
            S32AND(xr1, xr1, xr15);
            S32AND(xr3, xr3, xr15);
            D32SLR(xr1,xr1,xr3,xr3,1); // xr1 = a>>1, xr3 = b>>1

            S32AND(xr5, xr5, xr15);
            S32AND(xr7, xr7, xr15);
            D32SLR(xr5,xr5,xr7,xr7,1); // xr5 = c>>1, xr7 = d>>1

            //(xr* & 0xE79CE79C)>>2
            S32AND(xr2, xr1, xr15);
            S32AND(xr4, xr3, xr15);
            D32SLR(xr2,xr2,xr4,xr4,1); // xr2 = a>>2, xr4 = b>>2

            S32AND(xr6, xr5, xr15);
            S32AND(xr8, xr7, xr15);
            D32SLR(xr6,xr6,xr8,xr8,1); // xr6 = c>>2, xr8 = d>>2

            //*dst = ((a & 0xE79CE79C)>>2) + ((a & 0xF7DEF7DE)>>1) +  ((b & 0xE79CE79C)>>2);
            D32ACC_AA(xr1,xr2,xr4,xr0);
            S32STD(xr1,dst,0);
           
	        //*(dst+120) = ((b & 0xF7DEF7DE)>>1) + ((c & 0xF7DEF7DE)>>1);
	        D32ADD_AA(xr3,xr3,xr5,xr0);
            S32STD(xr3,dst,120*4);
            
	        //*(dst+120*2) = ((c & 0xC718C718)>>2) + ((d & 0xF7DEF7DE)>>1) + ((d & 0xE79CE79C)>>2);
            D32ACC_AA(xr6,xr7,xr8,xr0);
            S32STD(xr6,dst,120*8);
...
/*convert 224px to 160px using MXU instructions by drowsnug */
void downscale_224to160_mxu(uint32_t* __restrict__ src, uint32_t* __restrict__ dst)
{
    ...
    const uint16_t iy=7;
    const uint32_t mask[2]  = {0xF7DEF7DE,  0xE79CE79C};   
    ...
    for(int H=0;H<32;H+=1)
    {
        ...
        for(int W=0;W<120;W++) 
        {
            ...
            S32LDI(xr5,buffer_mem,512); //c=buffer_mem[x+128*2];
            S32LDI(xr7,buffer_mem,512); //d=buffer_mem[x+128*3];
            S32LDI(xr9,buffer_mem,512); //e=buffer_mem[x+128*4];
            S32LDI(xr11,buffer_mem,512); //f=buffer_mem[x+128*5];
            S32LDI(xr13,buffer_mem,512); //g=buffer_mem[x+128*6];
            ...
            S32AND(xr9, xr9, xr15);
            S32AND(xr11, xr11, xr15);
            D32SLR(xr9,xr9,xr11,xr11,1); // xr9 = e>>1, xr11 = f>>1

            S32AND(xr13, xr13, xr15);
            D32SLR(xr13,xr13,xr0,xr0,1); // xr13 = g>>1

            //(xr* & 0xE79CE79C)>>2
            //S32LDD(xr15, mask, 0x4);
            S32AND(xr2, xr1, xr15);
            S32AND(xr4, xr3, xr15);
            D32SLR(xr2,xr2,xr4,xr4,1); // xr2 = a>>2, xr4 = b>>2

            S32AND(xr8, xr7, xr15);
            S32AND(xr12, xr11, xr15);
            S32AND(xr14, xr13, xr15);
            D32SLR(xr8,xr8,xr12,xr12,1); // xr8 = d>>2, xr12 = f>>2
            D32SLR(xr14,xr14,xr0,xr0,1); // xr14 = g>>2

            //(xr* & 00xC718C718)>>3
            S32LDD(xr15, mask, 0x4);
            S32AND(xr6, xr5, xr15);
            S32AND(xr10, xr9, xr15);
            D32SLR(xr6,xr6,xr10,xr10,2); // xr6 = c>>3, xr10 = e>>3

            //*dst = ((a & 0xF7DEF7DE)>>1) + ((a & 0xE79CE79C)>>2) + ((b & 0xE79CE79C)>>2);
            D32ACC_AA(xr1,xr2,xr4,xr0);
            S32STD(xr1,dst,0);
            ...
            D32ACC_AA(xr6,xr7,xr8,xr0);
            D32ACC_AA(xr6,xr10,xr0,xr0);
            S32STD(xr6,dst,120*8);
            
	        //*(dst+120*3) = ((e & 0xF7DEF7DE)>>1) + ((f & 0xF7DEF7DE)>>1);
            D32ADD_AA(xr9,xr9,xr11,xr0);
            S32STD(xr9,dst,120*12);
            
            //*(dst+120*4) = ((f & 0xE79CE79C)>>2) + ((g & 0xE79CE79C)>>2) + ((g & 0xF7DEF7DE)>>1);
            D32ACC_AA(xr12,xr13,xr14,xr0);
            S32STD(xr12,dst,120*16);
 	        dst++;
            x ++;
        }
        dst += 120*4;
